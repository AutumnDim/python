...
raster_path = r"F:\work\出图\数据\k\sum_num.tif"
...
deep_blue_cmap = 'hsv' #ListedColormap(['#0000FF', '#0000FF'])
img = ax1.imshow(
    data,
    extent=extent,
...
fig.text(0.51, 0.19, 'Area(km²)', ha='center', fontsize=15, color='black')  # 栅格数据标签
...
ps = pd.read_csv("F:\work\出图\ps.csv").iloc[:,1:4]
x = np.array(ps['1']+5)
y = np.array(ps['0']*0.00001)
x = average_every_five_rows(x)
y = average_every_five_rows(y)
...
cmap = 'jet'  # 使用Blues颜色映射
#colors = color_map(np.cos(x)[:-1], cmap)  
colors = color_map(y[:-1], cmap)  

line_segments = LineCollection(segments, colors=colors, linewidths=1, linestyles='solid', cmap=cmap)  # 创建线段集合

# 绘制副图
ax2.set_ylim(np.min(x)-4, np.max(x)+0.1)  # 设置X轴范围
ax2.set_xlim(np.min(y)+1, np.max(y)+0.5)  # 设置Y轴范围
...
ax2.set_xlim([np.min(y) - 0.05, np.max(y) + 0.05])
# 添加箭头
...
# #计算1公里网格里面像元个数
# path = r"F:\work\出图\K.tif"
# window_id = 0  
# kist = []
# for k in range(0,2852):
#     with rasterio.open(path) as src:   #(285195,218803)
#         height = src.height
#         width = src.width
#         window_height = 100
#         window_width = 218803
#         w = src.read(1, window=Window(0,window_height*k, 218803, 100))
#         block_rows = 100
#         block_cols = 100
#         cist = []
#         for i in range(2188):
#             start_col = i * block_cols
#             end_col = start_col + block_cols
#             sub_block = w[:, start_col:end_col]  
#             count = np.sum(sub_block == 1)
#             cist.append(count)
#         kist.append(cist)
# matrix = np.array(kist)


# with rasterio.open("F:\work\出图\KK.tif") as okk:
#     cnpp_data = okk.read(1).astype('float')  # 读取第一波段
#     nodata = okk.nodata
#     cnpp_data[cnpp_data == nodata] = np.nan
#     meta = okk.meta
# meta.update(dtype='float32', count=1, nodata=np.nan)
# with rasterio.open(r"F:\work\出图\sum_num.tif", 'w', **meta) as dest:
#         dest.write(matrix, 1)
...
# # 计算并绘制副图
# lat_values = np.linspace(bottom, top, 2852)
# x = lat_values
# y = np.array(kist)
