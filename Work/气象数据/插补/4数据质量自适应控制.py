...
def QA_2(df2, prcp, data_qa, path_era5):
    ...
    result_all, accuracy_all, r2_all = Common_interpolation(era5, data_qa, df2, prcp_id, value, None, None)
...
if __name__ in '__main__':
    ...
    path_prcp = r"C:\降水归档\降水数据插值(国内)_气象局\3数据插值\1缺失值插值\prcp_data\prcp_*.csv"      # 插值后的降水数据
    path = r"C:\降水归档\降水数据插值(国内)_气象局\2数据质量控制\插值后质量判断\QC_prcp_bin.csv"          # 插值后的质量判断
    path_nan = r"C:\降水归档\降水数据插值(国内)_气象局\1数据缺失值概况\多年缺失值占比.csv"           # 缺失值占比数据
    path_lon = r"C:\降水归档\降水数据插值(国内)_气象局\0降水数据\格式整理数据\unique_station_1980_2022.txt"  # 经纬度数据
