...
def clean_data(data_all):
    ...
    data_all[(data_all > 10000) | (data_all < -999)] = np.nan
    return data_all
...
def process_group(group_data):
    ...
    mean_accuracy, y_pre, group1, missing, target_col = QW_hanshu.Naive_Bayes_classifier(group, day, feature, target,
                                                                               random_seed=42)
    if y_pre is None:
        return group1, mean_accuracy
    else:
        y_pre1 = y_pre + 2000    #TODO2025/5/14
...
def process_station(task):
    ...
    # 验证插值精度
    r2 = QW_hanshu.validate_kriging(grouped_all, target, xi, yi, x1, y1, elevation1)
    r2 = pd.DataFrame({f'{target}': [r2]})

    # 进行泛克里金插值
    result = QW_hanshu.interpolate_values(grouped_all, target, xi, yi, x1, y1, elevation1)

    return result, accuracies, r2

def main():
    path = r"E:\QX\插补数据\sta.txt"                 # 经纬度数据
    path_qa = r"E:\QX\插补数据\TMAX\2\QC_tmax_bin.csv"                    # 质量判断数据
    path_data = r"E:\QX\插补数据\TMAX\TMAX_*.txt"             # 原始降水量数据
    path_nan = r"E:\QX\插补数据\TMAX\1\多年缺失值占比.csv"                       # 缺失值占比数据
    ...
    output_path = r"E:\QX\插补数据\TMAX\3"
    ...
    # 获取质量信息数据
    data_qa = pd.read_csv(path_qa, dtype=str, index_col=0,encoding_errors='ignore')
    data_qa = data_qa.T             #TODO 2025/5/13
    data_qa = data_qa[data_qa.index.isin(nan)]          #TODO 2025/5/13
    data_qa = data_qa.T             #TODO 2025/5/13
    ...
    # --- 确保 result_all 的索引是时间序列 ---
    if not isinstance(result_all.index, pd.DatetimeIndex):
        # 如果有 prcp_id，就用它的时间索引
        if 'prcp_id' in locals() and isinstance(prcp_id.index, pd.DatetimeIndex):
            result_all.index = prcp_id.index
        else:
            # 否则就用你之前生成的 date_range
            result_all.index = pd.DatetimeIndex(date_range)
    # 分年输出结果
    QW_hanshu.Output_annually(result_all,df2,output_path)
