...
def acquire_GHCND(path,start_year,end_year):
    """
    获取GHCND数据
    """
    GHCND = pd.DataFrame()
    for y in range(start_year,end_year):
        path_GHCND = path + os.sep + f'PRCP_{y}.csv'
...
if __name__ == '__main__':
    ...
    path_data = r"D:\实习\降水量\precipitation_time_2020_*.txt"  # 原始降水量数据
    path_sta = r"/结果(最新版2)/克里金插值/low_r2_d.csv"  # R方低于0.8的数据
    path_qa = r"/结果(最新版2)/插值前质量判断/QC_prcp_bin.csv"  # 质量判断数据
    output_path = r"/结果(最新版2)/人工神经网络(MSWEP版)"  # 数据输出路径
    path_GHCND = r"/atm/data/station/Station_data/PRCP1"
    path_MSWEP = r'/MSWEP'
    ...
    QA = filter_years(QA,start_year,end_year)
    GHCND = acquire_GHCND(path_GHCND,start_year,end_year)

    all_year = pd.DataFrame()
    all_8d = pd.DataFrame()
    all_d = pd.DataFrame()
    yearly_8d = pd.DataFrame()
    yearly_d = pd.DataFrame()
    ...
    for name in tqdm(sta.index,total=len(sta.index),desc='Procession stations'):
        ...
        prcp = filter_years(prcp,start_year,end_year)
        ...
        # 获取GPM数据
        df_all = pd.concat(
            [pd.read_csv(path_MSWEP + os.sep +f"PRCP_{y}.csv",encoding_errors='replace')[name].rename(lambda x: f'{y}-{x + 1}') for y in range(start_year, end_year)]
        )
        ...
        reg_day = reg_day.clip(lower=0, upper=182.88)
        prcp_data = pd.concat([prcp_data,reg_day],axis=1)
        ...
        merge_d_y = merge_d.groupby(merge_d.index.year).apply(
            lambda x: calculate_stats(x[f'CH0000{name}'], x[name])
        )
        ...
        yearly_8d = pd.concat([yearly_8d, merge_8d_y_df])
        yearly_d = pd.concat([yearly_d, merge_d_y_df])
    ...
    yearly_8d.to_csv(output_path + os.sep + 'evaluate_yearly_8d.csv')
    yearly_d.to_csv(output_path + os.sep + 'evaluate_yearly_d.csv')

    prcp_data.to_csv(output_path + os.sep + 'prcp_86.csv')
