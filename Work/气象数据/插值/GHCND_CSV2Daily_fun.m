...
for js = 1 : 68
    ...
    while(~feof(fip))
        ...
        ymd = str2double(cell2mat(s1{1,2}));
        
        dy = ymd - fix(ymd / 100) * 100;
        yr_pro = fix(ymd / 100 / 100);
        mn = fix((ymd - yr_pro * 100 * 100 - dy) / 100);
        jday = fix(datenum(yr_pro, mn, dy) - datenum(yr_pro, 1, 1) + 1);
        for i = 1 : n1
            sid = s1{1,1}{i,1};
            mvr = s1{1,3}{i,1};
            v = fix(find(strcmp(vname,mvr)));
            jd = jday(i);
            
            if ~ghcn.isKey(sid) && jd > 0 && v > 0 && v < v_num
