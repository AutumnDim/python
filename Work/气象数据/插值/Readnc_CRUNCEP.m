...
% 
%         for i = 0 : natts-1
%             % (3) Get more information about the dimensions, variables, and global attributes in the file by using netCDF inquiry functions.
%             global_att_name = netcdf.inqAttName(ncid,netcdf.getConstant('NC_GLOBAL'),i);
% 
%             % (4) Get the value of the attribute, using the netcdf.getAtt function.
%             global_att_value = netcdf.getAtt(ncid,netcdf.getConstant('NC_GLOBAL'),global_att_name);
% 
%             % (5) Get information about the dimensions defined in the file through a series of calls to netcdf.inqDim.
%             [xtype attlen] = netcdf.inqAtt(ncid,netcdf.getConstant('NC_GLOBAL'),global_att_name);
% 
%             disp([global_att_name ': ' global_att_value]);
%         end
% 
%         % (6) Get information about the variables in the file through a series of calls to netcdf.inqVar.
        for i = 0 : ndims-1
            [dimname, dimlen(i+1)] = netcdf.inqDim(ncid,i);
            % disp([dimname ' ' num2str(dimlen(i+1))]);
        end

        % (7)  Retrieve the data from the variables and attributes use the netcdf.getVar function 
        dat = netcdf.getVar(ncid,0);
        lat = netcdf.getVar(ncid,1);
        lon = netcdf.getVar(ncid,2);
        ntm = netcdf.getVar(ncid,3);
        netcdf.close(ncid);
